---
title: SEO, JSON-LD, RSS & Sitemap
description: Generate page metadata, JSON-LD structured data, an XML sitemap, RSS/Atom feeds, and llms.txt for a Next.js Markdown blog with @next-md-blog/core.
---
...
If a post's frontmatter has **`faq`** or **`howto`**, `schemaGraph` automatically appends a Schema.org **`FAQPage`** and/or **`HowTo`** node to the `@graph` — nothing else to call. See [Content & frontmatter](/content-and-frontmatter#faq-and-howto-structured-data-since-v13) for the frontmatter shapes.

```tsx
// post.frontmatter.faq / .howto set → graph includes FAQPage / HowTo
const jsonLd = blog.schemaGraph(post);
// → { "@graph": [Organization, BlogPosting, BreadcrumbList, FAQPage, HowTo] }
```
