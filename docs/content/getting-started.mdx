...
`@next-md-blog/core` v1.2 is organized around **collections**. A collection owns one content surface (blog, glossary, docs, changelog, …) with its own folder, URL segment, schema type, and RSS feed. A single shared **`SiteConfig`** holds site-wide concerns (name, URL, Organization JSON-LD).
