...
Deploy like any Next.js site. Set **`NEXT_PUBLIC_SITE_URL`** to the live origin in production so canonical and Open Graph URLs match your domain. Combine **`generateStaticParams`** with **`collection.getAll()`** for static post pages when appropriate.
