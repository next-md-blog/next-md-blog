---
title: Markdown & MDX Blog for the Next.js App Router
description: Build a Markdown or MDX blog on the Next.js App Router. Filesystem posts, frontmatter-only metadata, and built-in JSON-LD, RSS, sitemap & SEO helpers — no sidecar meta files or content database.
---
...
**Markdown-first content for [Next.js](https://nextjs.org)**. Keep posts as `.md` / `.mdx` on disk. Declare any number of **collections** (blog, glossary, docs, changelog…) via **`defineCollection`**, render with **`MarkdownContent`**, and ship **metadata**, **JSON-LD**, **RSS**, **sitemap**, and **llms.txt** helpers — plus an optional **CLI** to scaffold routes, config, and **`sitemap / robots / RSS`** files.
...
flowchart LR
  ...
  subgraph next["Next.js App Router"]
    ...
    MDX["MarkdownContent"]
    SEO["collection.metadata + schemaGraph"]
  end
  MD --> CORE
  CFG --> CORE
  RSC --> CORE
...
Install [`@next-md-blog/core`](https://www.npmjs.com/package/@next-md-blog/core), keep your posts as `.md` / `.mdx` files on disk, declare a collection with `defineCollection`, and render it with `MarkdownContent` from a Server Component. The optional [CLI](/cli) scaffolds the routes, config, and SEO files for you. See [Getting started](/getting-started).
...
No. Posts live as Markdown files in your repository and metadata lives in each file's frontmatter — there is no database, no sidecar `post.json`, and no generated content manifest to keep in sync. See [Comparison](/comparison) for how this differs from CMS-backed and codegen-based stacks.

### Does it handle SEO, sitemap, RSS, and JSON-LD?

Yes. The core library ships metadata, JSON-LD (`schemaGraph`), RSS/Atom feeds, `sitemap`, `robots`, and `llms.txt` helpers, and the CLI can generate the matching routes. See [SEO & feeds](/seo-and-feeds).

### Does it support internationalization (i18n)?

Yes — locale segments and per-locale content folders are supported. See [Internationalization](/internationalization) and the [i18n demo](https://demo.i18n.next-md-blog.com).

## Where to go next

- [Getting started](/getting-started) — install, CLI, first routes
- [CLI reference](/cli) — flags, **`seo`** subcommand, **`--force`**
- [Configuration](/configuration) — `defineSite`, `defineCollection`, and site settings
