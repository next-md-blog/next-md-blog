---
title: CLI reference
description: Scaffold routes, config, sitemap, robots, and RSS for a Next.js Markdown blog with @next-md-blog/cli — flags, the seo subcommand, and --force.
---
...
- **`app/sitemap.ts`** (or **`src/app/sitemap.ts`**) — every collection via **`composeSitemap`**
- **`app/robots.ts`** — **`getRobots(site)`**
- **`app/feed.xml/route.ts`** — RSS via **`blog.rssResponse()`**
...
1. Writes **`next-md-blog.config.ts`** with **`defineSite`** + **`defineCollection`** from **`@next-md-blog/core`**. Add more collections (glossary, docs…) by calling **`defineCollection`** again.
