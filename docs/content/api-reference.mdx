...
The 1.2 surface is organized around `defineCollection`. Site-wide helpers (Organization JSON-LD, robots, sitemap composition) take a `SiteConfig`.
...
- **`<MarkdownContent />`** — React Server Component that renders a `BlogPost.content` string. Accepts `components`, `remarkPlugins`, `rehypePlugins`.
